//! Exercises for 2.5.4 — designing an error taxonomy for a service.
//!
//! `ValidationError` and `ServiceError` are given fully written — their
//! *shape* is what `## مفهوم`/`## The concept` walked through, and getting
//! that shape right is the point of reading this lesson, not of typing it.
//! What is `todo!()` below is the logic that produces and consumes them.
...
fn validate(title: &str, rating: u8) -> Result<(), ValidationError> {
    todo!(
        "if the trimmed title is empty, return ValidationError::EmptyTitle; otherwise, if \
         rating is greater than 10, return ValidationError::RatingOutOfRange with that rating; \
         otherwise return Ok"
    )
}
...
impl WatchlistStore {
    ...
    /// at `0`.
    pub fn new() -> Self {
        todo!("start with an empty map of entries and a next id of 0")
    }
    ...
    pub fn add_entry(&mut self, title: &str, rating: u8) -> Result<u64, ServiceError> {
        todo!(
            "validate title and rating, propagating any failure; on success, insert a new \
             Entry under the next id, advance the counter, and return the id that was used"
        )
    }
    ...
    pub fn rating_of(&self, id: u64) -> Result<u8, ServiceError> {
        todo!("look up id and return its rating, or ServiceError::NotFound with that id")
    }
    ...
    pub fn restore_from_file(&mut self, path: &str) -> Result<usize, ServiceError> {
        todo!(
            "read the file at path (letting a failure become ServiceError::Internal on its \
             own); for each non-blank line shaped title=rating whose rating parses as a u8, add \
             a new entry and count it; skip any line that is not shaped that way; return the \
             count"
        )
    }
}
...
mod tests {
    ...
    fn restore_from_file_adds_well_shaped_lines_and_skips_the_rest() {
        let mut store = WatchlistStore::new();
        let path = std::env::temp_dir().join("p2-05-04-restore-well-shaped.txt");
        std::fs::write(
            &path,
            "Frieren=10\nBocchi=8\n\nnot-a-pair\nAlso=notanumber\n",
        )
    ...
    fn restore_from_file_reports_a_missing_file_as_internal() {
        let mut store = WatchlistStore::new();
        let err = store
            .restore_from_file("p2-05-04-this-path-does-not-exist.txt")
            .unwrap_err();
