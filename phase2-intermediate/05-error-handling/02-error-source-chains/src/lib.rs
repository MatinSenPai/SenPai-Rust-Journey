...
impl From<std::io::Error> for ConfigError {
    fn from(source: std::io::Error) -> Self {
        todo!("wrap `source` in the ConfigError variant that holds an io::Error")
    }
}
...
impl Error for ConfigError {
    ...
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        todo!(
            "Some(the wrapped error) for the Io and InvalidNumber variants; None for \
             MissingField, since it has no deeper cause"
        )
    }
}
...
pub fn load_config(path: &str) -> Result<Config, ConfigError> {
    todo!(
        "read the file at `path` with std::fs::read_to_string, propagating a read failure with \
         ?, then pass its contents to parse_config_str and return that result"
    )
}
...
pub fn error_chain(err: &dyn Error) -> Vec<String> {
    todo!(
        "build the Vec described above: err's own message, then each .source() link's message, \
         in order, stopping when a link's source is None"
    )
}
...
pub fn effective_max_retries(
    ...
    todo!(
        "if override_max_retries holds Some text, parse it as u32 and propagate a parse \
         failure with ?, without touching path; if it is None, call load_config(path), \
         propagate its failure with ?, and return max_retries from the result"
    )
}
