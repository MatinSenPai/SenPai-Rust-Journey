...
pub fn describe(value: &Cow<'_, str>) -> &'static str {
    todo!("say whether `value` holds a borrowed or an owned string")
}
...
pub fn collapse_spaces(input: &str) -> Cow<'_, str> {
    todo!("collapse every run of 2+ consecutive spaces into a single space, exactly as described above")
}
...
pub fn ensure_exclaimed(mut value: Cow<'_, str>) -> Cow<'_, str> {
    todo!("make sure `value` ends with '!', adding one only when it is missing")
}
