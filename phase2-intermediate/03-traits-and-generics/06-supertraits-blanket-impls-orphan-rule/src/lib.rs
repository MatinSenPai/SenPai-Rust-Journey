//! Exercises for 2.3.6 — supertraits, blanket impls, the orphan rule, and the
//! newtype escape hatch.
//!
...
pub trait Discounted: Priced {
    ...
    fn discounted_cents(&self) -> u32 {
        todo!("return price_cents() multiplied by 90, divided by 100, using integer math")
    }
}
...
pub fn amount_saved<T: Discounted>(item: &T) -> u32 {
    todo!("subtract item's discounted_cents() from its price_cents()")
}
...
impl Cart {
    ...
    pub fn total_cents(&self) -> u32 {
        todo!("add up price_cents() for every Pen in self.0")
    }
}
...
impl std::fmt::Display for Cart {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        todo!("write the pen count and self.total_cents() to f, in the form shown above")
    }
}
