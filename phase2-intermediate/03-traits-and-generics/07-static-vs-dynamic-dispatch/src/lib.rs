...
pub fn total_summary_length_generic<T: Summarize>(items: &[T]) -> usize {
    todo!("add up the length, in bytes, of every item's summary text")
}
...
pub fn lineup(series: AnimeSeries, volume: MangaVolume) -> Vec<Box<dyn Summarize>> {
    todo!("build a Vec containing `series` first, then `volume`, each boxed as a trait object")
}
...
pub fn fibonacci() -> impl Iterator<Item = u64> {
    Fibonacci {
        current: todo!("the first value the sequence should produce"),
        next: todo!("the second value the sequence should produce"),
    }
}
