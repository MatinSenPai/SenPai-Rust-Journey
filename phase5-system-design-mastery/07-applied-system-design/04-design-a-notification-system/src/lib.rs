...
impl NotificationStore {
    ...
    pub async fn create(
        ...
        todo!(
            "sqlx::query_scalar::<_, i64>(\"INSERT INTO p5_07_04_notifications (user_id, \
             message, read) VALUES ($1, $2, false) RETURNING id\").bind(&input.user_id)\
             .bind(&input.message).fetch_one(&self.pool).await, mapping sqlx::Error to \
             NotificationError::Storage(e.to_string()); build a Notification from the new id + \
             input.user_id + input.message + read: false; call \
             self.channel_for(&notification.user_id).send(notification.clone()) and ignore its \
             Result (a send error just means nobody's subscribed right now, which is fine — \
             the notification is already saved); return Ok(notification): {{}}"
        )
    }

    pub async fn list_unread(&self, user_id: &str) -> Result<Vec<Notification>, NotificationError> {
        todo!(
            "sqlx::query_as::<_, Notification>(\"SELECT id, user_id, message, read FROM \
             p5_07_04_notifications WHERE user_id = $1 AND read = false ORDER BY id\")\
             .bind(user_id).fetch_all(&self.pool).await, mapping sqlx::Error to \
             NotificationError::Storage(e.to_string()): {{}}"
        )
    }

    pub async fn mark_read(&self, id: i64) -> Result<(), NotificationError> {
        todo!(
            "sqlx::query(\"UPDATE p5_07_04_notifications SET read = true WHERE id = $1\")\
             .bind(id).execute(&self.pool).await, mapping sqlx::Error to \
             NotificationError::Storage(e.to_string()); check .rows_affected() == 0 and return \
             Err(NotificationError::NotFound) in that case, otherwise Ok(()): {{}}"
        )
    }
}
