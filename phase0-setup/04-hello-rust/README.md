...
**Time:** ~45 minutes · **Prerequisites:** [03 — Cargo basics](../03-cargo-basics/README.md)
...
```sh
cargo run -p p0-04-hello-rust --example 01-hello
cargo run -p p0-04-hello-rust --example 02-formatting
...
```sh
cargo run -p p0-04-hello-rust --example 04-wrong-return --features broken
```
...
```sh
cargo test -p p0-04-hello-rust
```

Then check the real program works too:

```sh
cargo run -p p0-04-hello-rust
```
...
```sh
cargo run -p p0-04-hello-rust > out.txt
```
