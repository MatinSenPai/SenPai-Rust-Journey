//! Drives the whole axum stack through `tower::ServiceExt::oneshot`, exactly
//! like Phase 3's anime catalog CRUD lesson's `api_test.rs`.
...
use sq_03_webtoon_notifier_service::{
    app, AlwaysNewChapterChecker, AppState, NeverNewChapterChecker, WebtoonStore,
};
...
async fn checking_a_webtoon_with_a_new_chapter_updates_it_and_reports_true() {
    ...
    let webtoon = store.follow(sq_03_webtoon_notifier_service::FollowWebtoon {
