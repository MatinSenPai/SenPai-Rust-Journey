...
//! structured logging. See `README.md` for the tour.
//!
//! Everything down to [`TokenBucket::refill`] is given, fully working. Four
//! `todo!()`s are left: [`TtlCache::get`], [`TtlCache::set`],
//! [`get_or_fetch`] (the cache-aside orchestration — the heart of this
//! project), and [`TokenBucket::refill`] (the rate limiter's refill math,
//! reused/adapted from `phase4-backend-advanced/02-rate-limiting-and-backpressure/01-token-bucket-and-tower-limit`).
...
impl TtlCache {
    ...
    pub fn get(&self, id: u64) -> Option<TitleInfo> {
        todo!(
            "lock self.store, look up `id`; if present AND self.clock.now() is before the \
             stored expiry Instant, return Some(info.clone()); otherwise (absent OR expired) \
             return None"
        )
    }
    ...
    pub fn set(&self, id: u64, info: TitleInfo) {
        todo!(
            "lock self.store, insert (info, self.clock.now() + self.ttl) under `id` \
             (HashMap::insert overwrites unconditionally, which is correct here — a fresh set \
             should always win)"
        )
    }
}
...
{
    todo!(
        "if cache.get(id) is Some(info), return (Some(info), true) — a hit, compute() must not \
         run at all; otherwise call compute().await: if it returns Some(info), call \
         cache.set(id, info.clone()) and return (Some(info), false); if it returns None, return \
         (None, false) without caching anything — there's nothing to cache for a title that \
         doesn't exist"
    )
}
