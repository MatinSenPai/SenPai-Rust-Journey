...
//! `InMemoryCache` and `RedisCache` (the real thing).
...
/// A real Redis-backed `Cache`, so you can see what production code
/// against the `redis` crate looks like. `#[ignore]`d tests below exercise
/// this against a genuinely live Redis — see `README.md` for how to run
/// them.
...
mod tests {
    ...
    /// then:
    ///   cargo test --manifest-path solution/Cargo.toml -- --ignored
    #[tokio::test]
    #[ignore]
