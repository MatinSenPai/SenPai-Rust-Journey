...
pub async fn process_order(order_id: &str, amount_cents: u64) -> Result<(), OrderError> {
    todo!(
        "emit tracing::info!(\"processing order\") to mark the start, then \
         charge_payment(order_id, amount_cents).await? (the ? propagates a decline \
         without calling send_confirmation), then send_confirmation(order_id).await, \
         then emit tracing::info!(\"order processing complete\"), then return Ok(())"
    )
}
...
async fn charge_payment(order_id: &str, amount_cents: u64) -> Result<(), OrderError> {
    todo!(
        "if amount_cents == 0, emit tracing::warn!(\"refusing to charge a zero-amount \
         order\") and return Err(OrderError::PaymentDeclined) built from order_id \
         converted to a String; otherwise emit \
         tracing::info!(amount_cents, \"payment charged\") (a structured field, not a \
         format!-interpolated string) and return Ok(())"
    )
}
...
async fn send_confirmation(order_id: &str) {
    todo!("emit tracing::info!(\"confirmation sent\")")
}
...
pub fn build_subscriber(json: bool) -> Box<dyn tracing::Subscriber + Send + Sync> {
    todo!(
        "build a tracing_subscriber::fmt() builder with \
         .with_env_filter(tracing_subscriber::EnvFilter::from_default_env()) so RUST_LOG \
         still works; if json is true, chain .json() before .finish(), otherwise call \
         .finish() directly; wrap whichever result in Box::new(...) so both branches \
         return the same type (the two branches are different concrete SubscriberBuilder \
         types, so impl Subscriber alone would not compile here)"
    )
}
