//! Exercises for 1.2.1 — Stack and heap.
//!
...
//!
//! `std::mem::size_of::<T>()` and `std::mem::size_of_val(&value)` are the two
//! tools you need. Both were used in `examples/01-sizes.rs`.
...
pub fn stack_sizes() -> (usize, usize, usize) {
    todo!("ask the standard library how large each of those three types is")
}
...
pub fn capacity_after_pushes(pushes: usize) -> usize {
    todo!("build the Vec by pushing that many values, then report its capacity")
}
...
pub fn reserve_for(expected: usize) -> Vec<u8> {
    todo!("make a Vec that has asked for room up front")
}
...
pub fn header_and_heap(text: String) -> (usize, usize) {
    todo!("measure the value itself, then ask how much text it holds")
}
...
pub fn total_bytes(values: Vec<i64>) -> usize {
    todo!("add the size of the value itself to the size of the room it reserved")
}
