//! Exercises for 1.2.3 — `Clone` and `Copy`.
//!
...
pub fn duplicated(text: String) -> (String, String) {
    todo!("hand back the original and a copy that owns its own buffer")
}
...
pub fn array_survives(values: [i32; 4]) -> ([i32; 4], i32) {
    todo!("add them up and return the array alongside the total")
}
...
pub fn shrunk(values: Vec<i32>) -> Vec<i32> {
    todo!("produce a Vec holding the same values with no spare room")
}
...
pub fn doubled_up(values: Vec<String>) -> Vec<String> {
    todo!("for each string, put two of it into the answer")
}
...
pub fn repeated(text: String, times: usize) -> Vec<String> {
    todo!("build the Vec, and think about how many copies you actually need")
}
