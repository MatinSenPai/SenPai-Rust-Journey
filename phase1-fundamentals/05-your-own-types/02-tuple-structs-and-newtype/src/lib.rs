//! Exercises for 1.5.2 — Tuple structs and the newtype pattern.
//!
//! Three wrappers, each around a number you already know how to store. The
//! types are written for you; the constructors, accessors and the functions
//! that speak in those types are not.
//!
//! Run `cargo test -p p1-05-02-tuple-structs-and-newtype`.
...
impl Rial {
    ...
    pub fn new(amount: i64) -> Rial {
        todo!("wrap the given integer up as a Rial")
    }
    ...
    pub fn amount(self) -> i64 {
        todo!("hand the wrapped integer back to the caller")
    }
}
...
impl Percent {
    ...
    pub fn new(value: u8) -> Percent {
        todo!("store the value, refusing to hold anything above 100")
    }
    ...
    pub fn value(self) -> u8 {
        todo!("hand the wrapped number back to the caller")
    }
    ...
    pub fn of(self, amount: Rial) -> Rial {
        todo!("work out this percentage of the amount and return it as a Rial")
    }
}
...
pub fn total(amounts: &[Rial]) -> Rial {
    todo!("add every amount together and return the sum as a Rial")
}
...
pub fn transfer(from: AccountId, to: AccountId, amount: Rial) -> String {
    todo!("build the record line from the two account numbers and the amount")
}
