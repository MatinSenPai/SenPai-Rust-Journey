//! Exercises for 1.6.4 — panic versus `Result`.
//!
...
pub fn parse_priority(input: &str) -> Result<u8, String> {
    todo!("trim and parse input as a u8, checking it falls in 1..=5, per the doc comment")
}
...
pub fn priority_label(level: u8) -> &'static str {
    todo!("match level 1..=5 to a word per the doc comment; unreachable!() otherwise")
}
...
pub fn checked_midpoint(sorted_ascending: &[i32]) -> i32 {
    todo!("assert the slice is non-empty, naming the invariant, then index len() / 2")
}
...
pub fn last_digit_of(values: &[u32]) -> u32 {
    todo!("expect() a non-empty values with a message naming the invariant, then % 10")
}
