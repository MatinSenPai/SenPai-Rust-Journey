//! Exercises for 1.6.2 — `Option` combinators.
//!
//! Six small functions. Each one is naturally a single combinator (or a
//! short chain of them) rather than a `match`. Read the doc comment before
//! you touch the body — it says exactly what is expected.
...
pub fn shout(word: Option<&str>) -> Option<String> {
    todo!("transform the string inside Some, leaving None as None")
}
...
pub fn safe_half(n: Option<i32>) -> Option<i32> {
    todo!("halve n, but the halving itself can say \"no result\" — pick the combinator whose closure is allowed to return an Option, not just a plain value")
}
...
pub fn positive_only(n: Option<i32>) -> Option<i32> {
    todo!("keep n only if a condition on it holds")
}
...
pub fn take_and_reset(slot: &mut Option<String>) -> Option<String> {
    todo!("empty out slot and hand back whatever was in it")
}
...
pub fn coordinates(x: Option<i32>, y: Option<i32>) -> Option<(i32, i32)> {
    todo!("pair x and y up, but only when both sides are present")
}
...
pub fn first_available(primary: Option<i32>, backup: Option<i32>) -> Option<i32> {
    todo!("fall back to backup only when primary is absent")
}
