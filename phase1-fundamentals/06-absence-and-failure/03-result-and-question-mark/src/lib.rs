//! Exercises for 1.6.3 — `Result` and the question mark.
//!
...
pub fn safe_divide(a: f64, b: f64) -> Result<f64, String> {
    todo!("return Err when b is 0.0, otherwise return Ok of the division")
}
...
pub fn parse_quantity(s: &str) -> Result<u32, String> {
    todo!(
        "parse s as a u32; on failure, turn the parse error into a String using its own error text"
    )
}
...
pub fn chained_division(a: f64, b: f64, c: f64) -> Result<f64, String> {
    todo!("call safe_divide twice, feeding the first result into the second, and stop early on the first failure")
}
...
pub fn half_or_zero(input: &str) -> f64 {
    todo!("parse input as an f64, halve it on success, and fall back to 0.0 on any parse failure")
}
...
pub fn parsed_and_divided(input: &str, divisor: f64) -> Option<f64> {
    todo!("parse input as an f64, then divide it by divisor, turning either failure into None")
}
