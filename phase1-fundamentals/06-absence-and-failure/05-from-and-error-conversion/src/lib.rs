//! Exercises for 1.6.5 — `From` and error conversion.
//!
...
impl From<ParseIntError> for ColorError {
    fn from(err: ParseIntError) -> Self {
        todo!("wrap `err` in ColorError's channel-parsing variant")
    }
}
...
pub fn parse_color(s: &str) -> Result<Color, ColorError> {
    todo!(
        "split s into three comma-separated channels, parse each as a u8 with `?`, then check \
         the darkness rule before building the Color"
    )
}
...
impl From<Celsius> for Fahrenheit {
    fn from(value: Celsius) -> Self {
        todo!("apply f = c * 9/5 + 32 to value.0 and wrap the result in Fahrenheit")
    }
}
...
pub fn all_fahrenheit(values: Vec<Celsius>) -> Vec<Fahrenheit> {
    todo!("build a new Vec the same length as values, converting each Celsius to Fahrenheit and keeping the order")
}
