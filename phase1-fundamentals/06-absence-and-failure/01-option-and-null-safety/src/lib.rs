//! Exercises for 1.6.1 — `Option` and null safety.
//!
...
pub fn index_of_first_negative(readings: [i32; 6]) -> Option<usize> {
    todo!(
        "walk the array by position; as soon as a reading is negative, hand back its position \
         wrapped so the type says \"found\"; if the loop finishes with nothing negative, hand \
         back a value of the same type that says \"not found\""
    )
}
...
pub fn safe_average(total: i32, count: u32) -> Option<f64> {
    todo!("guard the zero-count case first; otherwise divide total by count as f64 values")
}
...
pub fn nickname_len(nickname: &Option<String>) -> Option<usize> {
    todo!(
        "look inside the reference without moving the String out of it, and report its byte \
         length if there is one"
    )
}
...
pub fn first_word_upper(words: &[String]) -> Option<String> {
    todo!("find the first entry, if there is one, and hand back an upper-cased owned copy of it")
}
...
pub fn greeting(profile: &Profile) -> String {
    todo!("match on the profile's nickname field and produce one of the two exact greetings")
}
