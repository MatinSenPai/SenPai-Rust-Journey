//! Exercises for 1.4.4 — Slicing text safely.
//!
...
pub fn char_boundaries(text: &str) -> Vec<usize> {
    todo!("collect every byte index of `text` at which a cut is legal, smallest first")
}
...
pub fn safe_prefix(text: &str, max_bytes: usize) -> &str {
    todo!("return the longest prefix of `text` that fits `max_bytes` bytes and ends on a boundary")
}
...
pub fn truncate_to_chars(text: &str, max_chars: usize) -> &str {
    todo!("return the first `max_chars` characters of `text`, counted as characters")
}
...
pub fn truncated_with_ellipsis(text: &str, max_chars: usize) -> String {
    todo!("shorten `text` to `max_chars` characters, adding an ellipsis only if you removed some")
}
...
pub fn split_at_char(text: &str, char_index: usize) -> (&str, &str) {
    todo!("cut `text` in two after `char_index` characters and return both halves")
}
