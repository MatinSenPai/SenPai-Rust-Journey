...
```sh
DATABASE_URL=postgres://taskforge:taskforge@localhost:5432/p3_05_04_transactions \
  sqlx migrate revert --source phase3-backend-foundations/05-postgres-and-sqlx/04-transactions/migrations
```
...
```sh
sudo -u postgres psql -c "CREATE DATABASE p3_05_04_transactions OWNER taskforge;"
```
...
`cargo test -p p3-05-04-transactions` (the error-type and
validate-before-connect tests run with zero infrastructure), then:

```sh
DATABASE_URL=postgres://taskforge:taskforge@localhost:5432/p3_05_04_transactions \
  cargo test -p p3-05-04-transactions -- --ignored
```
