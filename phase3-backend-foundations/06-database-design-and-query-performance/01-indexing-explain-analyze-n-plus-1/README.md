...
1. **`group_joined_rows`** — the pure regrouping logic. No database
   involved; `cargo test -p p3-06-01-indexing-explain-analyze-n-plus-1`
...
DATABASE_URL=postgres://taskforge:taskforge@localhost:5432/taskforge \
  cargo test -p p3-06-01-indexing-explain-analyze-n-plus-1 -- --ignored
```

## Next

`cargo test -p p3-06-01-indexing-explain-analyze-n-plus-1`, then
