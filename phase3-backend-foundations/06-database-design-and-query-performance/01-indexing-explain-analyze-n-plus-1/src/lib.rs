...
pub fn group_joined_rows(rows: Vec<JoinedRow>) -> Vec<PostWithComments> {
    todo!(
        "walk `rows` in order, keeping a Vec<PostWithComments> plus a way to find 'have I already \
         started this post_id' (e.g. a HashMap<i64, usize> from post_id to its index in the Vec); \
         for each row, if post_id hasn't been seen yet push a new PostWithComments with an empty \
         comments Vec and record its index, then if comment_id is Some(id) push a Comment built \
         from comment_id/post_id/comment_body.unwrap() onto that post's comments Vec; return the Vec"
    )
}
...
impl BlogStore {
    ...
    /// `05-postgres-and-sqlx/02-migrations`) — this toy inlines `CREATE
    ...
    pub async fn posts_with_comments_naive(&self) -> Result<Vec<PostWithComments>, sqlx::Error> {
        todo!(
            "call self.list_posts().await? to get every post (that's query #1), then for each \
             post in that Vec call self.comments_for_post(post.id).await? (one more query per \
             post — this loop *is* the N+1), pushing PostWithComments {{ post, comments }} onto a \
             Vec you build up and return as Ok(..) at the end"
        )
    }
...
mod tests {
    ...
    ///     cargo test -p p3-06-01-indexing-explain-analyze-n-plus-1 -- --ignored --test-threads=1
    #[tokio::test]
    #[ignore]
