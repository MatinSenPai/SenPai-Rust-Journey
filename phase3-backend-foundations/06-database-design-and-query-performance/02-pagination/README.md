...
1. **`encode_cursor` / `decode_cursor`** — pure functions, no database.
   Plain `cargo test -p p3-06-02-pagination` exercises them, including the
...
`#[serial(p3_06_02_pagination_db)]` on each — same reasoning as 05.1):

```sh
DATABASE_URL=postgres://taskforge:taskforge@localhost:5432/taskforge \
  cargo test -p p3-06-02-pagination -- --ignored
```

## Next

`cargo test -p p3-06-02-pagination`, then the recall questions, then
